...
function fromDbFormat(dbAppointment: DbAppointment): Appointment {
  ...
  const startDateTime = `${dbAppointment.appointment_date}T${dbAppointment.start_time}`;
  const endDateTime = `${dbAppointment.appointment_date}T${dbAppointment.end_time}`;
...
export const appointmentService = {
  ...
  async getAppointments(params: {
    ...
    return (data || []).map(item => {
      ...
      if (item.doctor && item.doctor.users) {
        appointment.doctor.name = item.doctor.users.full_name;
      }
